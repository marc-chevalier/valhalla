...
static const Type* make_constant_from_flat_array_element(ciFlatArray* array, int off, int field_offset, int stable_dimension,
                                                         BasicType loadbt, bool is_unsigned_load) {
  int nm_offset = array->element_type()->as_inline_klass()->null_marker_offset_in_payload();
  if (!array->is_null_free() && field_offset != nm_offset) {
    ciConstant null_marker_value = array->field_value_by_offset(off + nm_offset);
    if (null_marker_value.basic_type() != T_BOOLEAN || !null_marker_value.as_boolean()) {
      return nullptr;  // Cannot get null marker or it is not true (so element is not stabilized yet)
    }
  }
