...
void ClassFileParser::fill_instance_klass(InstanceKlass* ik,
  ...
  if (is_inline_type()) {
      ...
      for (int i = 0; i < ikao.vk->total_fields_count(); ++i) {
        ...
        if (fd.is_flat()) {
          ...
          if (LayoutKindHelper::is_nullable_flat(ili.kind())) {
            ...
            if (UseNewCode) tty->print_cr("  null marker mask piece 0x%016lx", mask_piece);
            mask |= mask_piece;
          }
          ...
          if (!is_java_primitive(bt)) {
            ...
            if (UseNewCode) tty->print_cr("  mask piece 0x%016lx", mask_piece);
            mask |= mask_piece;
          }
        }
    ...
    if (UseNewCode) tty->print_cr("mask: 0x%016lx\n", mask);
