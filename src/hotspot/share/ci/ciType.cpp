...
ciWrapper::ciWrapper(ciType* type, int properties)
  ...
  assert(!(type->is_instance_klass() && !type->is_inlinetype()) || type->as_instance_klass()->flags().is_identity() || type->as_instance_klass()->flags().is_abstract(), "An instance klass, without identity and that is not abstract?!");
