    ...
    if (UseHashcodeFastPath && !_gvn.type(obj)->is_inlinetypeptr()) {
      ...
      if (!stopped()) {
        ...
        if (!stopped()) {
          ...
          if (!stopped()) {
            Node* const empty_result = get_hashcode_from_header(klass_header, slow_region);
            RegionNode* const unmasked_region = new RegionNode(4);
            Node* const unmasked_result = new PhiNode(unmasked_region, TypeInt::INT);

            Node* const bol_empty_object = BoolCmpI(offset, BoolTest::eq, zerocon(T_INT));
            IfNode* const iff_is_empty_object = create_and_map_if(control(), bol_empty_object, PROB_FAIR, COUNT_UNKNOWN);
            unmasked_result->init_req(1, empty_result);
            ...
            IfNode* const iff_is_long_payload = create_and_map_if(control(), is_long_payload_bol, PROB_FAIR, COUNT_UNKNOWN);
            Node* const int_result = AddI(MulI(intcon(31), empty_result), ConvL2I(obj_extracted));
            unmasked_region->init_req(2, IfFalse(iff_is_long_payload));
            unmasked_result->init_req(2, int_result);

            Node* const long_result = AddI(MulI(intcon(31), int_result), ConvL2I(URShiftL(obj_extracted, intcon(32))));
            unmasked_region->init_req(3, IfTrue(iff_is_long_payload));
            unmasked_result->init_req(3, long_result);

            Node* const fast_path_result = AndI(_gvn.transform(unmasked_result), intcon(markWord::hash_mask));
