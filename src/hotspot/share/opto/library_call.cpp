  ...
  if (base_type->isa_instptr()) {
    ...
    if (layout == LayoutKind::REFERENCE) {
      if (!base_type->is_aryptr()->is_not_flat()) {
        ...
        Node* new_base = _gvn.transform(new CheckCastPPNode(control(), base, array_type, ConstraintCastNode::DependencyType::NonFloatingNarrowing));
      ...
      if (UseArrayFlattening) {
        ...
        if (ptr_type->field_offset().get() != 0) {
          ptr = _gvn.transform(new CheckCastPPNode(control(), ptr, ptr_type->with_field_offset(0), ConstraintCastNode::DependencyType::NonFloatingNarrowing));
        }
      } else {
        uncommon_trap(Deoptimization::Reason_intrinsic,
